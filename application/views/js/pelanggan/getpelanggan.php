...
function gambar(foto){
    ...
    $("#gPreview").attr("src", base_url + 'assets/upload/pelanggan/' + foto);
}
...
function hapusAll() {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Pelanggan/hapus_all";
        }
    });
...
function getPelanggan(id) {
    ...
    $.ajax({
        ...
        success: function(hasil) {
            $('#idpelanggan').val(hasil[0].pelanggan_id);
            $('#noktp').val(hasil[0].pelanggan_noktp);
            $('#namaL').val(hasil[0].pelanggan_nama);
            $('#alamat').val(hasil[0].pelanggan_alamat);
            $('#pekerjaan').val(hasil[0].pelanggan_pekerjaan);
            $("#kelamin").val(hasil[0].pelanggan_kelamin).trigger("chosen:updated");
            enable();
        }
    });
}
...
$(document).ready(function() {
    ...
    table = $('#table-1').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('pelanggan/getDataPelanggan')?>",
            "type": "POST"
        },
