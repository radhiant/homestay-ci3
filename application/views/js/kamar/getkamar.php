...
function konfirmasi(id) {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Kamar/proses_hapus/" + id;
        }
    });
...
function hapusAll() {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Kamar/hapus_all";
        }
    });
...
function getKamar(id) {
    ...
    var base_url = link + 'kamar/getData';
    ...
    $.ajax({
        ...
        success: function(hasil) {
            $('#idkamar').val(hasil[0].kamar_id);
            $('#nokamar').val(hasil[0].kamar_no);
            $('#type').val(hasil[0].kamar_type);
            $('#nilaiU').val(formatRupiah(hasil[0].kamar_biaya, ''));
            enable();
        }
    });
}
...
$(document).ready(function() {
    ...
    table = $('#table-1').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('kamar/getDataKamar')?>",
            "type": "POST"
        },
