...
function konfirmasi(id) {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Smbrbiaya/proses_hapus/" + id;
        }
    });
...
function hapusAll() {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Smbrbiaya/hapus_all";
        }
    });
...
function getsmbrbiaya(id) {
    disable();
    var link = $('#baseurl').val();
    var base_url = link + 'Smbrbiaya/getData';
    ...
    $.ajax({
        ...
        success: function(hasil) {
            $('#idsmbrbiaya').val(hasil[0].smbrbiaya_id);
            $('#namaK').val(hasil[0].smbrbiaya_nama);
            $('#ket').val(hasil[0].smbrbiaya_ket);
            enable();
        }
    });
}
...
$(document).ready(function() {
    ...
    table = $('#table-1').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('Smbrbiaya/getDatasmbrbiaya')?>",
            "type": "POST"
        },
