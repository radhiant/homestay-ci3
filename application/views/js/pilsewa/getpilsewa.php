...
function konfirmasi(id) {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Pilsewa/proses_hapus/" + id;
        }
    });
...
function hapusAll() {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Pilsewa/hapus_all";
        }
    });
...
function getpilsewa(id) {
    disable();
    var link = $('#baseurl').val();
    var base_url = link + 'Pilsewa/getData';
    ...
    $.ajax({
        ...
        success: function(hasil) {
            $('#idpilsewa').val(hasil[0].pilsewa_id);
            $('#ps').val(hasil[0].pilsewa_nama);
            $('#type').val(hasil[0].pilsewa_type);
            $('#harga').val(formatRupiah(hasil[0].pilsewa_harga, ''));
            $('#ket').val(hasil[0].pilsewa_ket);
            enable();
        }
    });
}
...
$(document).ready(function() {
    ...
    table = $('#table-1').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('Pilsewa/getDatapilsewa')?>",
            "type": "POST"
        },
