...
function konfirmasi(id) {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Sales/proses_hapus/" + id;
        }
    });
...
function hapusAll() {
    ...
    swal.fire({
        ...
        if (result.value) {
            loadingklik();
            window.location.href = base_url + "Sales/hapus_all";
        }
    });
...
function getSales(id) {
    ...
    var base_url = link + 'sales/getData';
    ...
    $.ajax({
        ...
        success: function(hasil) {
            $('#idsales').val(hasil[0].sales_id);
            $('#namaS').val(hasil[0].sales_nama);
            $('#notelp').val(hasil[0].sales_notelp);
            $('#tglbayar').val(hasil[0].sales_tgl_bayar);
            $('#biayaS').val(hasil[0].sales_biaya);
            $('#ket').val(hasil[0].sales_ket);
            enable();
        }
    });
}
...
$(document).ready(function() {
    ...
    table = $('#table-1').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('sales/getDataSales')?>",
            "type": "POST"
        },
